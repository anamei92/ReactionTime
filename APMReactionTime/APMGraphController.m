...
@interface APMGraphController ()

@end

@implementation APMGraphController
...
-(void)configurePlots {
    ...
    aaplPlot.identifier = @"Reaction Time";
...
-(void)configureAxes {
    ...
    CGFloat dateCount = [[[APMData sharedInstance] times] count];
    NSMutableSet *xLabels = [NSMutableSet setWithCapacity:dateCount];
    NSMutableSet *xLocations = [NSMutableSet setWithCapacity:dateCount];
    NSInteger i = 0;
    for (NSString *date in [[APMData sharedInstance] times]) {
        NSString *inStr = [NSString stringWithFormat:@"%d", i];
        CPTAxisLabel *label = [[CPTAxisLabel alloc] initWithText:inStr  textStyle:x.labelTextStyle];
        CGFloat location = i++;
...
-(NSUInteger)numberOfRecordsForPlot:(CPTPlot *)plot {
    return [[[APMData sharedInstance] times] count];
}

-(NSNumber *)numberForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)index {
    NSInteger valueCount = [[[APMData sharedInstance] times] count];
    switch (fieldEnum) {
        ...
        case CPTScatterPlotFieldY:
            return [[[APMData sharedInstance] times] objectAtIndex:index];
